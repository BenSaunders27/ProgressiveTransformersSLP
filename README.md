...
Prepare Phoenix14T (or other sign language dataset) data as .txt files for .skel, .gloss, .txt and .files. Data format should be parallel .txt files for src, trg and files, with each line representing a new sequence. trg file should contain skeleton data, with a space separating joint values and each frame containing joints and counter to the length of trg_size length. Files file should contain the name of each sequence on a new line. 
